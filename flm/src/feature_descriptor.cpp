...
{
	assert(shape.rows == 1 && eyeIndices.size() == 4);
	...
	float lx = (shape.at<float>(eyeIndices.at(0)) + shape.at<float>(eyeIndices.at(1))) * 0.5;
	float ly = (shape.at<float>(eyeIndices.at(0) + numLandmarks) + shape.at<float>(eyeIndices.at(1) + numLandmarks)) * 0.5;
	float rx = (shape.at<float>(eyeIndices.at(2)) + shape.at<float>(eyeIndices.at(3))) * 0.5;
	float ry = (shape.at<float>(eyeIndices.at(2) + numLandmarks) + shape.at<float>(eyeIndices.at(3) + numLandmarks)) * 0.5;
	float distance = sqrt((rx - lx) * (rx - lx) + (ry - ly) * (ry - ly));
	int patch_width_half = std::round(hogParam.relativePatchSize * distance);

	cv::Mat hogDescriptors; // We'll get the dimensions later from vl_hog_get_*
	for (int i = 0; i < landmarkIndices.size(); i++)
	{
		int x = cvRound(shape.at<float>(landmarkIndices.at(i)));
		int y = cvRound(shape.at<float>(landmarkIndices.at(i) + numLandmarks));
		...
		int fixed_roi_size = hogParam.numCells * hogParam.cellSize;
		...
		VlHog *hog = vl_hog_new(VlHogVariant::VlHogVariantUoctti, hogParam.numBins, false);			 // transposed (=col-major) = false
		vl_hog_put_image(hog, (float *)roiImg.data, roiImg.cols, roiImg.rows, 1, hogParam.cellSize); // (the '1' is numChannels)
